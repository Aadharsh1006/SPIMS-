...
import {
    LogOut, Building2, Users, BarChart3,
    Megaphone, ShieldCheck, Menu, Crown, MessageSquare
} from 'lucide-react';

const navItems = [
    { to: '/superadmin/colleges', icon: Building2, label: 'Colleges' },
    { to: '/superadmin/tpos', icon: ShieldCheck, label: 'TPO Management' },
    { to: '/superadmin/recruiters', icon: Users, label: 'Manage Recruiters' },
    { to: '/superadmin/analytics', icon: BarChart3, label: 'System Analytics' },
    { to: '/superadmin/broadcasts', icon: Megaphone, label: 'Global Broadcast' },
    { to: '/superadmin/messages', icon: MessageSquare, label: 'Messages' },
];
            ...
            <div className="p-6 border-b border-[var(--border-main)] shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-black tracking-tighter text-[var(--text-main)] uppercase italic">
                        SPIMS<span className="text-violet-500">+</span>
                    </h1>
                </div>
                <p className="text-[10px] text-violet-400 font-black uppercase tracking-widest mt-1 ml-10">
                    Super Admin
                </p>
            </div>
                    ...
                    <Link
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl transition-all font-bold group
                            ${isActive(to)
                                ...
                                : 'text-[var(--text-muted)] hover:bg-violet-600/10 hover:text-violet-400'
                            }`}
                    >
                        <Icon
                            size={20}
                            className={isActive(to) ? 'text-white' : 'text-slate-400 group-hover:text-violet-400'}
                        />
                        {label}
                    </Link>
                ))}
    ...
    return (
        <div className="flex h-screen bg-[var(--bg-main)] text-[var(--text-main)] overflow-hidden transition-colors duration-300">
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                ...
                {/* Broadcast Banner sits above header */}
                <BroadcastBanner />
            ...
            <header className="h-16 bg-[var(--bg-card)]/80 backdrop-blur-md border-b border-[var(--border-main)] px-4 md:px-8 flex items-center justify-between z-50 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xs md:text-sm font-black text-violet-400 uppercase tracking-widest">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="w-8 h-8 rounded-full bg-violet-600 border border-violet-500/40 flex items-center justify-center">
